...
from app.leads.models import CrmCompanyIdentity, Lead, MessageRetryAttempt
...
def test_pending_create_discard_is_logical_and_idempotent(
    ...
    with session_factory.begin() as session:
        lead = Lead(
            ...
            smart_table_record_id="record-to-keep",
    ...
    assert lead.smart_table_record_id == "record-to-keep"
    assert len(audits) == 1
