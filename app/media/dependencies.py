...
def get_media_storage_provider(settings: Settings | None = None) -> StorageProvider:
    ...
    try:
        provider_result = get_provider_policy(selected).require(
...
def get_media_scanner(settings: Settings | None = None) -> FileScanProvider:
    ...
    if provider_result.provider == "noop":
        return NoopFileScanProvider()
    if provider_result.provider == "fake":
        return FakeFileScanProvider("clean")
    if provider_result.provider == "production":
...
def get_media_attachment_service(
    ...
    ocr_result = policy.require("ocr", settings.ocr_provider, settings=settings)
    asr_result = policy.require("asr", settings.asr_provider, settings=settings)
    api_key = settings.qwen_api_key or ""
    ocr = (
        MockOCRProvider([])
        if ocr_result.provider == "mock"
